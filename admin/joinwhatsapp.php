...
<body>
                  <?php
                    if (isset($_GET['action'], $_GET['id'])) {
                        ...
                        switch ($_GET['action']) {
                            case "delete":
                                $stm = $con->prepare("delete from joinwhatsspp where id=:id");
                                ...
                                break;
                                case "readen":
                                $stm = $con->prepare("update joinwhatsspp set readen=1 where id=:id");
                                $stm->execute(array("id"=>$id));
                                if($stm->rowCount()==1)
                                {
                                ...
                                break;
                                case "unreaden":
                                $stm = $con->prepare("update joinwhatsspp set readen=0 where id=:id");
                                $stm->execute(array("id"=>$id));
                                if($stm->rowCount()==1)
                                {
                    ...
                    <div class="col-md-9"style="width:100%">
                        <div class="card">
                            <div class="card-header d-flex justify-content-between align-items-center">
                                <h4 class="card-title"style="margin: auto;font-size:30px;">قائمة طلبات الانضمام الى الوتس اب </h4>
                                <div class="d-flex ">
                                </div>
                            </div>
                            <div class="card-body px-0 pb-0">
                                <div class="table-responsive">
                                    <table class='table mb-0' id="table1">
                                        ...
                                        <tbody>
                                        <?php
                                    $stm = $con->prepare("select * from joinwhatsspp");
                                    $stm->execute();
                                    if ($stm->rowCount()) {
